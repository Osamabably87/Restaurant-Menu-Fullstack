...
<div class="card border-0 shadow-lg rounded-4 overflow-hidden">
  ...
  <div class="card-body bg-white p-5">
    ...
    <form action="{{ route('update',$data->id) }}" method="POST" enctype="multipart/form-data">
      ...
      <!-- Back Button -->
      <div class="mb-4">
        <a href="{{ route('profile') }}" class="btn btn-secondary shadow-sm">← Back</a>
      </div>
      ...
      <div class="row mb-4">
        <div class="col-md-6">
          ...
          <input type="email" class="form-control form-control-lg rounded-3 shadow-sm" 
                 id="email" name="email" step="0.01" value="{{ $data->email }}" required>
                 @error('email')
                        <span class="text-danger">{{ $message }}</span>
                      @enderror
        </div>
      ...
      <div class="row mb-4">
        <div class="col-md-6">
          ...
          <input type="password" class="form-control form-control-lg rounded-3 shadow-sm" 
                 id="password" name="password" step="0.01"value="{{$data->password}}" required>
                 @error('password')
                        <span class="text-danger">{{ $message }}</span>
                      @enderror
        </div>
      ...
      <div class="row mb-4">
        <div class="col-md-6">
          ...
          <input type="password" class="form-control form-control-lg rounded-3 shadow-sm" 
                 id="password" name="password_confirmation" step="0.01" value="{{$data->password}}" required>
